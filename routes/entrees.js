...
router.post('/', (req, res) => {
  const {
    ...
    qte_carton = 0, qte_box = 0, qte_paquet = 0, qte_piece = 0,
  ...
  const qteTotale = totalPieces(produit, { carton: qte_carton, box: qte_box, paquet: qte_paquet, piece: qte_piece });
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(`
      INSERT INTO entrees (fournisseur_id, numero_bon, produit_id, qte_carton, qte_box, qte_paquet, qte_piece, qte_totale_pieces, prix_achat_unitaire, total, utilisateur_id, note)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(fournisseur_id || null, numero_bon || null, produit_id, qte_carton, qte_box, qte_paquet, qte_piece, qteTotale, prixUnitaire, total, req.user.id, note || null);
