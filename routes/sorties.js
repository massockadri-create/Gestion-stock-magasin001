...
router.post('/', (req, res) => {
  const {
    ...
    qte_carton = 0, qte_box = 0, qte_paquet = 0, qte_piece = 0, note
  } = req.body;
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(`
      INSERT INTO sorties (type_sortie, client_id, produit_id, qte_carton, qte_box, qte_paquet, qte_piece, qte_totale_pieces, utilisateur_id, note)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(type_sortie, client_id || null, produit_id, qte_carton, qte_box, qte_paquet, qte_piece, qteTotale, req.user.id, note || null);
