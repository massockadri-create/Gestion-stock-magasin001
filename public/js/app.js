...
// Chaîne d'unités par type de conditionnement, en miroir de utils.js côté serveur.
//  - cahier     : carton -> paquet -> pièce
//  - accessoire : carton -> box -> paquet -> pièce
//  - livre      : carton -> pièce (direct)
function unitLevels(p) {
  const type = p.type_conditionnement || 'cahier';
  const pp = Number(p.piece_par_paquet) || 1;
  if (type === 'livre') {
    return [{ key: 'carton', label: 'carton', pieces: Number(p.piece_par_carton) || 1 }];
  }
  if (type === 'accessoire') {
    const paquetParBox = Number(p.paquet_par_box) || 1;
    const boxParCarton = Number(p.box_par_carton) || 1;
    const piecesParBox = paquetParBox * pp;
    return [
      { key: 'carton', label: 'carton', pieces: boxParCarton * piecesParBox },
      { key: 'box', label: 'box', pieces: piecesParBox },
      { key: 'paquet', label: 'paquet', pieces: pp },
    ];
  }
  const paquetParCarton = Number(p.paquet_par_carton) || 1;
  return [
    { key: 'carton', label: 'carton', pieces: paquetParCarton * pp },
    { key: 'paquet', label: 'paquet', pieces: pp },
  ];
}

function unitStack(p, qty) {
  // affiche une quantité en pièces décomposée selon le conditionnement du produit
  const levels = unitLevels(p);
  let reste = qty;
  const parts = [];
  for (const lvl of levels) {
    const n = Math.floor(reste / lvl.pieces);
    reste -= n * lvl.pieces;
    if (n) parts.push(`<span class="unit-chip"><b>${n}</b> ${lvl.label}${n > 1 ? 's' : ''}</span>`);
  }
...
const LABEL_CONDITIONNEMENT = { cahier: 'Cahier (carton → paquet → pièce)', accessoire: 'Accessoire (carton → box → paquet → pièce)', livre: 'Livre (carton → pièce)' };
...
async function renderProduits(main, q = '') {
  ...
  main.innerHTML = `
    <div class="page-header">
      ...
      <div class="actions">
        <button class="btn secondary" id="btn-export-produits">⬇️ Exporter CSV</button>
        ${canEdit ? `<button class="btn secondary" id="btn-import-produits">⬆️ Importer CSV</button>` : ''}
    ...
    <div class="filters">
      ...
      <select id="filter-type"><option value="">Tous types</option><option value="cahier">📚 Cahiers</option><option value="accessoire">🖊️ Accessoires</option><option value="livre">📖 Livres</option></select>
    </div>
  ...
  async function refresh() {
    ...
    const tc = document.getElementById('filter-type').value;
    if (qv) params.set('q', qv);
    if (cat) params.set('categorie_id', cat);
    if (st) params.set('statut', st);
    if (tc) params.set('type_conditionnement', tc);
    ...
    list.querySelectorAll('[data-fiche]').forEach(b => b.addEventListener('click', () => openFicheProduit(b.dataset.fiche, canEdit)));
  }
  ...
  document.getElementById('btn-export-produits').addEventListener('click', async () => {
    try {
      const res = await fetch('/api/produits/export/csv', { headers: { Authorization: `Bearer ${state.token}` } });
  ...
  const importBtn = document.getElementById('btn-import-produits');
  if (importBtn) {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        const r = await api('POST', '/produits/import/csv', { csv: texte });
        let msg = `Import terminé : ${r.crees} créé(s), ${r.mis_a_jour} mis à jour`;
        if (r.erreurs.length) msg += `, ${r.erreurs.length} erreur(s)`;
        toast(msg, r.erreurs.length > 0);
        if (r.erreurs.length) console.warn('Erreurs import CSV :', r.erreurs);
        renderPage();
      } catch (err) { toast(err.message, true); }
      fileInput.value = '';
    });
...
// Génère les champs de conditionnement (dépendent du type sélectionné) + le script de bascule.
// `p` (optionnel) préremplit les valeurs en mode édition.
function champsConditionnementHTML(p) {
  const type = p?.type_conditionnement || 'cahier';
  return `
    <div class="field"><label>Type de conditionnement *</label>
      <select name="type_conditionnement" id="sel-conditionnement" required>
        <option value="cahier" ${type==='cahier'?'selected':''}>${LABEL_CONDITIONNEMENT.cahier}</option>
        <option value="accessoire" ${type==='accessoire'?'selected':''}>${LABEL_CONDITIONNEMENT.accessoire}</option>
        <option value="livre" ${type==='livre'?'selected':''}>${LABEL_CONDITIONNEMENT.livre}</option>
      </select>
    </div>
    <div class="field cond-fields cond-cahier cond-accessoire"><label>Pièces / paquet</label><input name="piece_par_paquet" type="number" value="${p?.piece_par_paquet ?? 10}"></div>
    <div class="field cond-fields cond-cahier"><label>Paquets / carton</label><input name="paquet_par_carton" type="number" value="${p?.paquet_par_carton ?? 10}"></div>
    <div class="field cond-fields cond-accessoire"><label>Paquets / box</label><input name="paquet_par_box" type="number" value="${p?.paquet_par_box ?? 10}"></div>
    <div class="field cond-fields cond-accessoire"><label>Box / carton</label><input name="box_par_carton" type="number" value="${p?.box_par_carton ?? 5}"></div>
    <div class="field cond-fields cond-livre"><label>Pièces / carton</label><input name="piece_par_carton" type="number" value="${p?.piece_par_carton ?? 20}"></div>
  `;
}

function activerBasculeConditionnement(ov) {
  const sel = ov.querySelector('#sel-conditionnement');
  function appliquer() {
    const type = sel.value;
    ov.querySelectorAll('.cond-fields').forEach(el => {
      el.style.display = el.classList.contains(`cond-${type}`) ? '' : 'none';
    });
  }
...
function openNouveauProduit() {
  ...
  `);
  activerBasculeConditionnement(ov);
...
async function openFicheProduit(id, canEdit) {
  const p = await api('GET', `/produits/${id}`);
  const catOptions = state.categoriesCache.map(c => `<option value="${c.id}" ${c.id===p.categorie_id?'selected':''}>${c.nom}</option>`).join('');
  ...
  const ov = modal(`
    ...
    ${canEdit ? `<div class="tabs-inline" id="fiche-tabs">
      <button data-tab="details" class="active">Détails</button>
      <button data-tab="modifier">Modifier</button>
    </div>` : ''}
    ...
    ${canEdit ? `<div id="tab-modifier" style="display:none">
      <form id="form-modifier-produit">
        <div class="form-grid">
          <div class="field"><label>Désignation</label><input name="designation" value="${p.designation}" required></div>
          <div class="field"><label>Pagination (nb pages)</label><input name="pagination" type="number" value="${p.pagination ?? ''}"></div>
          <div class="field"><label>Format</label><input name="format" value="${p.format ?? ''}"></div>
          <div class="field"><label>Marque</label><input name="marque" value="${p.marque ?? ''}"></div>
          <div class="field"><label>Type</label><input name="type_produit" value="${p.type_produit ?? ''}"></div>
          <div class="field"><label>Couverture</label><input name="couverture" value="${p.couverture ?? ''}"></div>
          <div class="field"><label>Catégorie</label><select name="categorie_id"><option value="">—</option>${catOptions}</select></div>
          ${champsConditionnementHTML(p)}
          <div class="field"><label>Seuil minimum (pièces)</label><input name="seuil_minimum" type="number" value="${p.seuil_minimum}"></div>
          <div class="field"><label>Prix d'achat (FCFA)</label><input name="prix_achat" type="number" value="${p.prix_achat}"></div>
          <div class="field"><label>Prix de vente (FCFA)</label><input name="prix_vente" type="number" value="${p.prix_vente}"></div>
        </div>
        <p class="hint" style="color:var(--ink-soft); font-size:12px; margin-top:6px">La référence et le stock actuel ne se modifient pas ici — utilise Entrées/Sorties pour ajuster le stock.</p>
        <div class="modal-actions">
          <button type="button" class="btn secondary" id="cancel-modif-produit">Annuler</button>
  ...
  if (canEdit) activerBasculeConditionnement(ov);
  ov.querySelector('#close-fiche').addEventListener('click', () => ov.remove());

  if (canEdit) {
    const tabs = ov.querySelector('#fiche-tabs');
    tabs.addEventListener('click', e => {
      if (e.target.tagName !== 'BUTTON') return;
      tabs.querySelectorAll('button').forEach(b => b.classList.remove('active'));
      e.target.classList.add('active');
      const showModifier = e.target.dataset.tab === 'modifier';
      ov.querySelector('#tab-details').style.display = showModifier ? 'none' : '';
      ov.querySelector('#tab-modifier').style.display = showModifier ? '' : 'none';
    });
    ov.querySelector('#cancel-modif-produit').addEventListener('click', () => ov.remove());
    ov.querySelector('#form-modifier-produit').addEventListener('submit', async e => {
      e.preventDefault();
      const fd = Object.fromEntries(new FormData(e.target));
      try {
        await api('PUT', `/produits/${id}`, fd);
...
// Champs de quantité (carton/box/paquet/pièce) — box et paquet s'affichent ou non
// selon le type de conditionnement du produit sélectionné dans le formulaire.
function champsQuantiteHTML() {
  return `
    <div class="field"><label>Quantité — cartons</label><input name="qte_carton" type="number" value="0"></div>
    <div class="field qte-field qte-box"><label>Quantité — box</label><input name="qte_box" type="number" value="0"></div>
    <div class="field qte-field qte-paquet"><label>Quantité — paquets</label><input name="qte_paquet" type="number" value="0"></div>
    <div class="field"><label>Quantité — pièces</label><input name="qte_piece" type="number" value="0"></div>
  `;
}

function activerBasculeQuantite(ov) {
  const sel = ov.querySelector('select[name="produit_id"]');
  function appliquer() {
    const p = state.produitsCache.find(x => String(x.id) === sel.value);
    const type = p?.type_conditionnement || 'cahier';
    ov.querySelectorAll('.qte-box').forEach(el => { el.style.display = type === 'accessoire' ? '' : 'none'; });
    ov.querySelectorAll('.qte-paquet').forEach(el => { el.style.display = type === 'livre' ? 'none' : ''; });
  }
  sel.addEventListener('change', appliquer);
  appliquer();
}
...
function openNouvelleEntree() {
  ...
  const ov = modal(`
    ...
    <form id="form-entree">
      ...
      <p class="hint" style="color:var(--ink-soft); font-size:12px; margin-top:6px">Le total en pièces est calculé automatiquement selon le conditionnement du produit choisi.</p>
      <div class="modal-actions">
...
async function renderFournisseurs(main) {
  ...
  main.innerHTML = `
    ...
    <div class="table-wrap card" style="padding:0"><table><thead><tr><th>Nom</th><th>Contact</th><th>Adresse</th><th></th></tr></thead><tbody>
      ${fournisseurs.map(f => `<tr><td>${f.nom}</td><td>${f.contact||'—'}</td><td>${f.adresse||'—'}</td><td>
        <button class="btn small secondary" data-mod="${f.id}">Modifier</button>
        <button class="btn small danger" data-del="${f.id}">Supprimer</button>
      </td></tr>`).join('') || `<tr><td colspan="4" style="text-align:center;color:var(--ink-soft);padding:24px">Aucun fournisseur</td></tr>`}
    </tbody></table></div>`;

  function formFournisseur(titre, f, onSubmit) {
    const ov = modal(`<h3>${titre}</h3><form id="f-form">
      <div class="form-grid"><div class="field"><label>Nom *</label><input name="nom" required value="${f?.nom ?? ''}"></div>
      <div class="field"><label>Contact</label><input name="contact" value="${f?.contact ?? ''}"></div>
      <div class="field span2"><label>Adresse</label><input name="adresse" value="${f?.adresse ?? ''}"></div></div>
    ...
    ov.querySelector('#f-form').addEventListener('submit', async e => {
      e.preventDefault();
      try { await onSubmit(Object.fromEntries(new FormData(e.target))); ov.remove(); renderPage(); }
      catch (err) { toast(err.message, true); }
    });
  }

  document.getElementById('btn-new-fournisseur').addEventListener('click', () => {
    formFournisseur('Nouveau fournisseur', null, async data => {
      await api('POST', '/fournisseurs', data);
      toast('Fournisseur ajouté');
    });
  });
  main.querySelectorAll('[data-mod]').forEach(b => b.addEventListener('click', () => {
    const f = fournisseurs.find(x => String(x.id) === b.dataset.mod);
    formFournisseur('Modifier le fournisseur', f, async data => {
      await api('PUT', `/fournisseurs/${f.id}`, data);
...
async function renderClients(main) {
  ...
  main.innerHTML = `
    ...
    <div class="table-wrap card" style="padding:0"><table><thead><tr><th>Nom</th><th>Type</th><th>Contact</th><th>Adresse</th><th></th></tr></thead><tbody>
      ${clients.map(c => `<tr><td>${c.nom}</td><td>${c.type_client}</td><td>${c.contact||'—'}</td><td>${c.adresse||'—'}</td><td>
        <button class="btn small secondary" data-mod="${c.id}">Modifier</button>
        <button class="btn small danger" data-del="${c.id}">Supprimer</button>
      </td></tr>`).join('') || `<tr><td colspan="5" style="text-align:center;color:var(--ink-soft);padding:24px">Aucun client</td></tr>`}
    </tbody></table></div>`;

  function formClient(titre, c, onSubmit) {
    const type = c?.type_client || 'particulier';
    const ov = modal(`<h3>${titre}</h3><form id="f-form">
      <div class="form-grid"><div class="field"><label>Nom *</label><input name="nom" required value="${c?.nom ?? ''}"></div>
      <div class="field"><label>Type</label><select name="type_client">
        <option value="particulier" ${type==='particulier'?'selected':''}>Particulier</option>
        <option value="commerce" ${type==='commerce'?'selected':''}>Commerce</option>
        <option value="librairie" ${type==='librairie'?'selected':''}>Librairie</option>
      </select></div>
      <div class="field"><label>Contact</label><input name="contact" value="${c?.contact ?? ''}"></div>
      <div class="field"><label>Adresse</label><input name="adresse" value="${c?.adresse ?? ''}"></div></div>
      <div class="modal-actions"><button type="button" class="btn secondary" id="c">Annuler</button><button class="btn" type="submit">Enregistrer</button></div>
    </form>`);
    ov.querySelector('#c').addEventListener('click', () => ov.remove());
    ov.querySelector('#f-form').addEventListener('submit', async e => {
  ...
  document.getElementById('btn-new-client').addEventListener('click', () => {
    formClient('Nouveau client', null, async data => {
      await api('POST', '/clients', data);
      toast('Client ajouté');
    });
  });
  main.querySelectorAll('[data-mod]').forEach(b => b.addEventListener('click', () => {
    const c = clients.find(x => String(x.id) === b.dataset.mod);
    formClient('Modifier le client', c, async data => {
      await api('PUT', `/clients/${c.id}`, data);
